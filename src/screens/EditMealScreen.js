...
import { analyzeFoodImage, reanalyzeFoodFromText, generateFoodImage } from '../services/geminiApi';
...
export default function EditMealScreen({ route, navigation }) {
  ...
  const [wwPoints,    setWwPoints]    = useState(String(meal.wwPoints          ?? 0));
  const [glucose,     setGlucose]     = useState(String(meal.glucoseRiseMgDl  ?? 0));
  ...
  // Sample image generated by Gemini for manual entries
  const [sampleImageUri, setSampleImageUri]     = useState(null);
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Persist the AI-generated sample image to the permanent meals directory
      let imageUri = meal.imageUri || undefined;
      if (sampleImageUri) {
        const mealsDir = FileSystem.documentDirectory + 'meals/';
        const dirInfo  = await FileSystem.getInfoAsync(mealsDir);
        if (!dirInfo.exists) await FileSystem.makeDirectoryAsync(mealsDir, { intermediates: true });
        const dest = mealsDir + `meal_${Date.now()}.jpg`;
        await FileSystem.copyAsync({ from: sampleImageUri, to: dest });
        imageUri = dest;
      }

      const updatedMeal = {
        ...meal,
        ...(pendingExtras || {}), // apply confidence, analyzedByModel if re-analyzed
        ...
        wwPoints:         parseInt(wwPoints,  10) || 0,
        glucoseRiseMgDl:  parseInt(glucose,   10) || 0,
  ...
  async function handleReanalyze() {
    ...
    try {
      ...
      setOriginalSnapshot({ foodName, calories, weight, protein, carbs, fat, wwPoints, glucose, ingredients, notes });
      ...
      setWwPoints(String(result.wwPoints         || 0));
      setGlucose (String(result.glucoseRiseMgDl  || 0));
      setIngredients(result.ingredients || []);
      setNotes   (result.notes || '');

      // Non-UI fields (confidence, model) applied on save
      setPendingExtras({
  ...
  function handleRevert() {
    ...
    setWwPoints   (originalSnapshot.wwPoints);
    setGlucose    (originalSnapshot.glucose);
  ...
  // ── Sample Image Generation (manual entry) ──

  /**
   * Calls the Gemini image generation model to produce a food photo matching
   * the current food name. Saves the base64 result to a temp cache file so it
   * can be displayed immediately and persisted on save.
   */
  async function handleGenerateImage() {
    if (!foodName.trim()) {
      Alert.alert('Food Name Required', 'Enter a food name so Gemini knows what to photograph.');
      return;
    }
    setGeneratingImage(true);
    try {
      const settings = await getSettings();
      const { base64, mimeType } = await generateFoodImage(foodName.trim(), settings.apiKey);
      const ext     = mimeType === 'image/png' ? 'png' : 'jpg';
      const tempUri = FileSystem.cacheDirectory + `sample_${Date.now()}.${ext}`;
      await FileSystem.writeAsStringAsync(tempUri, base64, {
        encoding: FileSystem.EncodingType.Base64,
      });
      setSampleImageUri(tempUri);
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Meal thumbnail — existing photo, or AI-generated sample for new entries */}
        {!!(sampleImageUri || meal.imageUri) && (
          <Image
            source={{ uri: sampleImageUri || meal.imageUri }}
            style={styles.image}
            resizeMode="cover"
          />
        )}
        ...
        <View style={styles.card}>
          ...
          <TextInput
            ...
            returnKeyType="done"
          />
          {/* AI Analyze + Sample Photo buttons — only for new manual entries */}
          {isNew && (
            <View style={styles.newEntryBtns}>
              <TouchableOpacity
                style={[styles.analyzeDescBtn, { flex: 1 }, reanalyzing && styles.reanalyzeBtnDisabled]}
                onPress={handleAnalyzeDescription}
                disabled={reanalyzing || saving || generatingImage}
                activeOpacity={0.75}
              >
                {reanalyzing
                  ? <ActivityIndicator color={colors.primary} />
                  : <Text style={styles.analyzeDescBtnText}>✨ Analyze</Text>
                }
              </TouchableOpacity>

              <TouchableOpacity
                style={[styles.samplePhotoBtn, generatingImage && styles.reanalyzeBtnDisabled]}
                onPress={handleGenerateImage}
                disabled={generatingImage || saving || reanalyzing}
                activeOpacity={0.75}
              >
                {generatingImage
                  ? <ActivityIndicator color={colors.primary} />
                  : <Text style={styles.samplePhotoBtnText}>
                      {sampleImageUri ? '🔄 New Photo' : '📷 Sample Photo'}
                    </Text>
                }
              </TouchableOpacity>
            </View>
        ...
        <View style={styles.card}>
          ...
          <View style={[styles.twoCol, { marginTop: spacing.md }]}>
            <NumericField label="WW Points"     value={wwPoints} onChange={setWwPoints} unit="pts"   accentColor="#1D4ED8" borderColor="#BFDBFE" />
            <NumericField label="Glucose Impact" value={glucose}  onChange={setGlucose}  unit="mg/dL" accentColor="#D97706" borderColor="#FED7AA" />
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  // AI Analyze + Sample Photo buttons row (for new manual entries)
  newEntryBtns:       { flexDirection: 'row', marginTop: spacing.sm, gap: spacing.sm },
  analyzeDescBtn:     { paddingVertical: spacing.sm, borderRadius: radius.md, borderWidth: 1.5, borderColor: colors.primary, borderStyle: 'dashed', alignItems: 'center', minHeight: 40, justifyContent: 'center' },
  samplePhotoBtn:     { flex: 1, paddingVertical: spacing.sm, borderRadius: radius.md, borderWidth: 1.5, borderColor: '#D97706', borderStyle: 'dashed', alignItems: 'center', minHeight: 40, justifyContent: 'center' },
  analyzeDescBtnText: { fontSize: fontSize.md, color: colors.primary, fontWeight: '700' },
  samplePhotoBtnText: { fontSize: fontSize.md, color: '#D97706', fontWeight: '700' },
});
